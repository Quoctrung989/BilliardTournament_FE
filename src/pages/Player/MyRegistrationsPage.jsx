                ...
                <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                  {items.map((row, index) => {
                    ...
                    return (
                      ...
                      <div className="ui-card group bg-white dark:bg-[#161a22] border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden flex flex-col w-full"
                        ...
                        <div className="p-4 flex flex-col gap-2.5 flex-1">
                          <button type="button" className="text-left"
                            ...
                            <h3 className="font-semibold text-slate-900 dark:text-white text-sm leading-[1.45] line-clamp-2 pb-[0.14em] hover:text-indigo-600 transition-colors">
                              {row.tournamentName}
                            </h3>
                          </button>
                        ...
                        <div className="px-4 py-2.5 border-t border-slate-100 dark:border-white/10 bg-slate-50/60 flex items-center gap-2">
                          ...
                          <button type="button" onClick={() => openDetail(row.id)}
                            className="ml-auto flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-600 dark:text-white/70 border border-slate-200 dark:border-white/10 bg-white dark:bg-[#161a22] hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                ...
                <div className="px-6 py-4 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 rounded-b-xl shrink-0 flex justify-end gap-2 flex-wrap">
                  {detail.status === "PENDING_PAYMENT" && (
                    <button type="button" onClick={() => setCancelModal({ id: detail.id })}
                      className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg border border-red-200 text-red-600 bg-white dark:bg-[#161a22] hover:bg-red-50 transition-colors cursor-pointer">
                      Hủy đăng ký
                    </button>
                  )}
                  <button type="button"
